...
QT += gui
QT -=xml
# this defines the OpenGL version we want to use for the loader gen code
# we use 4.4 on Linux / windows however mac will be either 3.2 or when maveriks arrives 4.x
# if this version doesnt work try 43 or 32 as a lowest version
USEGL=42
...
macx:{
        #SOURCES+=$$SRC_DIR/loaderGen/gl_OSXcore_3_2.c
        #HEADERS+=$$INC_DIR/gl_OSXcore_3_2.h
	QMAKE_CXXFLAGS+=  -fPIC
...
win32|unix:!macx{
	...
	unix:DEFINES += LINUX
	unix:LIBS+= -L/usr/lib64 -lGL -lGLU
	# and include the correct loader for GL version
	isEqual(USEGL, 44){
		message("configured to use  GL 44")
		SOURCES+=$$SRC_DIR/loaderGen/gl_core_4_4.c
		HEADERS+=$$INC_DIR/gl_core_4_4.h
		DEFINES+=GL44
	}
	isEqual(USEGL, 43){
		message("configured to use  GL 43")
		SOURCES+=$$SRC_DIR/loaderGen/gl_core_4_3.c
		HEADERS+=$$INC_DIR/gl_core_4_3.h
		DEFINES+=GL43
	}
	isEqual(USEGL, 42){
		message("configured to use  GL 42")
		SOURCES+=$$SRC_DIR/loaderGen/gl_core_4_2.c
		HEADERS+=$$INC_DIR/gl_core_4_2.h
		DEFINES+=GL42
	}
	isEqual(USEGL, 32){
		message("configured to use  GL 32")
		SOURCES+=$$SRC_DIR/loaderGen/gl_core_3_2.c
		HEADERS+=$$INC_DIR/gl_core_3_2.h
		DEFINES+=GL32
	}
}
