...
#include <string.h>
#include <stddef.h>
#include "gl_OSXcore_3_2.hpp"

#if defined(__APPLE__)
#include <dlfcn.h>
// modified from default based on https://bitbucket.org/alfonse/glloadgen/issue/29/compilation-errors-on-mac-os-x
...
namespace gl
{
	namespace exts
	{
		...
		LoadTest var_ARB_occlusion_query2;
		LoadTest var_ARB_shader_bit_encoding;
		LoadTest var_ARB_timer_query;
		LoadTest var_EXT_depth_bounds_test;
		LoadTest var_EXT_framebuffer_multisample_blit_scaled;
		LoadTest var_EXT_texture_mirror_clamp;
		LoadTest var_EXT_texture_sRGB_decode;
		LoadTest var_APPLE_client_storage;
		LoadTest var_APPLE_object_purgeable;
		LoadTest var_APPLE_rgb_422;
		LoadTest var_APPLE_row_bytes;
		LoadTest var_APPLE_texture_range;
	...
	namespace _detail
	{
		...
		typedef void (CODEGEN_FUNCPTR *PFNDEPTHBOUNDSEXT)(GLclampd, GLclampd);
		PFNDEPTHBOUNDSEXT DepthBoundsEXT = 0;

		static int Load_EXT_depth_bounds_test()
		{
			int numFailed = 0;
			DepthBoundsEXT = reinterpret_cast<PFNDEPTHBOUNDSEXT>(IntGetProcAddress("glDepthBoundsEXT"));
			if(!DepthBoundsEXT) ++numFailed;
			return numFailed;
		}

		typedef void (CODEGEN_FUNCPTR *PFNGETOBJECTPARAMETERIVAPPLE)(GLenum, GLuint, GLenum, GLint *);
		PFNGETOBJECTPARAMETERIVAPPLE GetObjectParameterivAPPLE = 0;
		typedef GLenum (CODEGEN_FUNCPTR *PFNOBJECTPURGEABLEAPPLE)(GLenum, GLuint, GLenum);
		PFNOBJECTPURGEABLEAPPLE ObjectPurgeableAPPLE = 0;
		typedef GLenum (CODEGEN_FUNCPTR *PFNOBJECTUNPURGEABLEAPPLE)(GLenum, GLuint, GLenum);
		PFNOBJECTUNPURGEABLEAPPLE ObjectUnpurgeableAPPLE = 0;

		static int Load_APPLE_object_purgeable()
		{
			int numFailed = 0;
			GetObjectParameterivAPPLE = reinterpret_cast<PFNGETOBJECTPARAMETERIVAPPLE>(IntGetProcAddress("glGetObjectParameterivAPPLE"));
			if(!GetObjectParameterivAPPLE) ++numFailed;
			ObjectPurgeableAPPLE = reinterpret_cast<PFNOBJECTPURGEABLEAPPLE>(IntGetProcAddress("glObjectPurgeableAPPLE"));
			if(!ObjectPurgeableAPPLE) ++numFailed;
			ObjectUnpurgeableAPPLE = reinterpret_cast<PFNOBJECTUNPURGEABLEAPPLE>(IntGetProcAddress("glObjectUnpurgeableAPPLE"));
			if(!ObjectUnpurgeableAPPLE) ++numFailed;
			return numFailed;
		}

		typedef void (CODEGEN_FUNCPTR *PFNGETTEXPARAMETERPOINTERVAPPLE)(GLenum, GLenum, GLvoid **);
		PFNGETTEXPARAMETERPOINTERVAPPLE GetTexParameterPointervAPPLE = 0;
		typedef void (CODEGEN_FUNCPTR *PFNTEXTURERANGEAPPLE)(GLenum, GLsizei, const GLvoid *);
		PFNTEXTURERANGEAPPLE TextureRangeAPPLE = 0;

		static int Load_APPLE_texture_range()
		{
			int numFailed = 0;
			GetTexParameterPointervAPPLE = reinterpret_cast<PFNGETTEXPARAMETERPOINTERVAPPLE>(IntGetProcAddress("glGetTexParameterPointervAPPLE"));
			if(!GetTexParameterPointervAPPLE) ++numFailed;
			TextureRangeAPPLE = reinterpret_cast<PFNTEXTURERANGEAPPLE>(IntGetProcAddress("glTextureRangeAPPLE"));
			if(!TextureRangeAPPLE) ++numFailed;
			return numFailed;
		}
	...
	namespace sys
	{
		namespace
		{
			...
			void InitializeMappingTable(std::vector<MapEntry> &table)
			{
				...
				table.push_back(MapEntry("GL_ARB_occlusion_query2", &exts::var_ARB_occlusion_query2));
				table.push_back(MapEntry("GL_ARB_shader_bit_encoding", &exts::var_ARB_shader_bit_encoding));
				table.push_back(MapEntry("GL_ARB_timer_query", &exts::var_ARB_timer_query, _detail::Load_ARB_timer_query));
				table.push_back(MapEntry("GL_EXT_depth_bounds_test", &exts::var_EXT_depth_bounds_test, _detail::Load_EXT_depth_bounds_test));
				table.push_back(MapEntry("GL_EXT_framebuffer_multisample_blit_scaled", &exts::var_EXT_framebuffer_multisample_blit_scaled));
				table.push_back(MapEntry("GL_EXT_texture_mirror_clamp", &exts::var_EXT_texture_mirror_clamp));
				table.push_back(MapEntry("GL_EXT_texture_sRGB_decode", &exts::var_EXT_texture_sRGB_decode));
				table.push_back(MapEntry("GL_APPLE_client_storage", &exts::var_APPLE_client_storage));
				table.push_back(MapEntry("GL_APPLE_object_purgeable", &exts::var_APPLE_object_purgeable, _detail::Load_APPLE_object_purgeable));
				table.push_back(MapEntry("GL_APPLE_rgb_422", &exts::var_APPLE_rgb_422));
				table.push_back(MapEntry("GL_APPLE_row_bytes", &exts::var_APPLE_row_bytes));
				table.push_back(MapEntry("GL_APPLE_texture_range", &exts::var_APPLE_texture_range, _detail::Load_APPLE_texture_range));
			}
			...
			void ClearExtensionVars()
			{
				...
				exts::var_ARB_occlusion_query2 = exts::LoadTest();
				exts::var_ARB_shader_bit_encoding = exts::LoadTest();
				exts::var_ARB_timer_query = exts::LoadTest();
				exts::var_EXT_depth_bounds_test = exts::LoadTest();
				exts::var_EXT_framebuffer_multisample_blit_scaled = exts::LoadTest();
				exts::var_EXT_texture_mirror_clamp = exts::LoadTest();
				exts::var_EXT_texture_sRGB_decode = exts::LoadTest();
				exts::var_APPLE_client_storage = exts::LoadTest();
				exts::var_APPLE_object_purgeable = exts::LoadTest();
				exts::var_APPLE_rgb_422 = exts::LoadTest();
				exts::var_APPLE_row_bytes = exts::LoadTest();
				exts::var_APPLE_texture_range = exts::LoadTest();
			}
